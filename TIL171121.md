...
- level1 에서 HONUX에게 배웠었지만, **`풀리퀘`** 를 보내는데에 까지 정확한 정보의 흐름을
  알지 못했다. **`merge`** 나 **`fetch`** 를 왜 해야 하는지에 대해 무지했고, 지금 내가 **`push`** 한 정보가 어디로 향해 가는지 / 왜 다른 **`branch`** 에서는 보이지 않는지 등에 대한 궁금증을 해소할 수 있었다.
